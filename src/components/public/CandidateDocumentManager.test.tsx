...
describe("CandidateDocumentManager", () => {
  ...
  it("opens the download target before awaiting the signed URL", async () => {
    ...
    await waitFor(() => expect(onDownload).toHaveBeenCalledWith(cvFixture));
    expect(openSpy).toHaveBeenCalledWith("about:blank", "_blank", "noopener,noreferrer");

    resolveDownload?.("https://signed.example/cv");
    await waitFor(() => expect(popup.location.href).toBe("https://signed.example/cv"));
    openSpy.mockRestore();
  });
});
