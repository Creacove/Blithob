...
export function AdminApplicationsPage({ repository = publicListingsRepository }: { repository?: PublicListingsRepository }) {
  ...
  const load = () => {
    setLoading(true);
    repository.listAdminApplications({ status: status === "all" ? undefined : status, jobId: jobId === "all" ? undefined : jobId, search, limit: 25, offset: 0 }).then(setApplications).catch((caught: unknown) => setError(caught instanceof Error ? caught.message : "Applications could not be loaded.")).finally(() => setLoading(false));
  };
  useEffect(() => {
    let active = true;
    repository
      .listAdminApplications({ status: status === "all" ? undefined : status, jobId: jobId === "all" ? undefined : jobId, search, limit: 25, offset: 0 })
      .then((rows) => {
        if (active) {
