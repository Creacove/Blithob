...
export function AdminApplicationsPage({ repository = publicListingsRepository }: { repository?: PublicListingsRepository }) {
  ...
  useEffect(() => { load(); }, [repository, status]);
  ...
  const convert = async (application: PublicApplication) => {
    ...
    setWorkingId(application.id); setError(null);
    try { await repository.convertApplication({ applicationId: application.id, agreedPayMinor: Math.round(amount * 100) }); load(); } catch (caught) { setError(caught instanceof Error ? caught.message : "Assignment could not be created."); } finally { setWorkingId(null); }
  };
